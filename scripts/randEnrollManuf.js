...
async function main() {
  ...
  try {
    ...
    // Randomize number of manufacturers to enroll (70-100)
    const minManufacturers = 70;
    const maxManufacturers = 100;
    const manufacturersToEnroll = Math.floor(Math.random() * (maxManufacturers - minManufacturers + 1)) + minManufacturers;
    
    console.log(`Enrolling ${manufacturersToEnroll} manufacturers...`);

    // Shuffle the wallets array to randomize selection
    const shuffledWallets = [...manufacturerWallets].sort(() => Math.random() - 0.5);
    
    for (let i = 0; i < Math.min(manufacturersToEnroll, shuffledWallets.length); i++) {
      try {
