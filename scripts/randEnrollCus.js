...
async function main() {
  ...
  try {
    ...
    // Randomize number of customers to enroll (70-100)
    const minCustomers = 70;
    const maxCustomers = 100;
    const customersToEnroll = Math.floor(Math.random() * (maxCustomers - minCustomers + 1)) + minCustomers;
    
    console.log(`Enrolling ${customersToEnroll} customers...`);

    // Shuffle the wallets array to randomize selection
    const shuffledWallets = [...customerWallets].sort(() => Math.random() - 0.5);
    
    for (let i = 0; i < Math.min(customersToEnroll, shuffledWallets.length); i++) {
      try {
